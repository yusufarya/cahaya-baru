          ...
          <table class="table table-bordered table-sm">
              ...
              <tbody>
                ...
                ?>
                  <tr>
                      {{-- <td><?= $row->code ?></td> --}}
                      <td><a href="/request-order/{{ $row->code }}/detail" class="text-primary px-2 py-1"> {{ $row->code }}</a></td>
                      <td>{{ $row->customers->fullname }}</td>
                      <td>{{ date('d - m - Y', strtotime($row->date)) }}</td>
                      <td style=" text-align: right;">{{ number_format($row->qty,2) }}</td> 
                      <td style=" text-align: right;">{{ number_format(($row->qty*$row->price)+$row->charge,2) }}</td> 
