...
<div class="explain-product my-4">
  <hr>
    ...
    <div class="row justify-content-center bg-secondary-color pb-3 px-3 mt-3 mx-auto w-75">
      ...
      <form id="submitForm" class="mt-3" action="registrant" method="GET">
        @csrf
        <select name="status" id="status" class="form-select form-control">
          <option value=""> Status</option>
          <option value="Y" {{ $status == 'Y' ? 'selected' : '' }}> Belum Dibayar</option>
          <option value="N" {{ $status == 'N' ? 'selected' : '' }}> Dibayar</option>
        </select>
